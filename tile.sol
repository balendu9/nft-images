...
contract Tiles {
    ...
    function setContract(address _regionNFT, address compute, address _user) external {
    ...
    function initializeRegion(uint256 regionId) external {
        ...
        for (uint8 i =0; i < GRID_SIZE; i++) {
            regionTiles[regionId][i] = TileData({
                id: i,
                isBeingUsed: false,
                isCrop: false,
                cropTypeId: 0,
    ...
    external onlyRegionOwner(regionId) {
        ...
        require(fertilzer >= 100, "NOT_ENOUGH_FERTILIZER");
